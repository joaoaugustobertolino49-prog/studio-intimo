...
// Upload de fotos para fal.ai storage
...
app.post('/api/upload', upload.array('images', 5), async (req, res) => {
  try {
    ...
    console.log(`[UPLOAD] ${urls.length} foto(s) enviadas.`);
    res.json({ urls });
  } catch (err) {
...
// Face swap: coloca rosto/corpo da pessoa no template de ensaio
app.post('/api/generate', async (req, res) => {
  try {
    ...
    console.log('[GENERATE] Iniciando face swap com Easel AI...');

    const resp = await fetch('https://fal.run/easel-ai/advanced-face-swap', {
      method: 'POST',
      headers: { 'Authorization': `Key ${key}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        base_image_url:  template_url,   // imagem do ensaio (template)
        face_image_0:    face_image_url, // foto da pessoa
        workflow_type:   'user_hair',    // preserva cabelo original da pessoa
      }),
    });

    const raw = await resp.text();
    console.log('[FAL STATUS]', resp.status);
    console.log('[FAL BODY]', raw.substring(0, 300));

    if (!resp.ok) {
      let msg = `Erro fal.ai ${resp.status}`;
      try { const e = JSON.parse(raw); msg = e.detail || e.message || e.error || msg; } catch(_) {}
    ...
    if (!url) {
      console.error('[FAL] Resposta sem URL:', JSON.stringify(data).substring(0, 200));
      return res.status(500).json({ error: 'Sem imagem na resposta. Tente novamente.' });
    }
